...
def make_postactivate_text(site_url):
    ...
    for key in environ.keys():
        if key.startswith(PREFIX):
            new_item = {key.replace(PREFIX, '', 1).lower(): environ.get(key)}
            settings.update(new_item)

    settings.update({
        'source_folder': '{root}/{url}/source'.format(
            root=WEBSERVER_ROOT, url=site_url, ),
        'site_url': site_url,
        'settings_module': '{module}.{version}'.format(
...
if (__name__) == '__main__':
    ...
    make_postactivate_file(site_url='local.{}'.format('example.com'),)
